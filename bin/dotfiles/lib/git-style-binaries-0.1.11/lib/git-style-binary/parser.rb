...
module GitStyleBinary
  class Parser < Trollop::Parser
    ...
    def spec_names
      @specs.collect { |_name, spec| spec[:long] }
    end
    ...
    def educate_long(stream = $stdout)
      ...
      if !@order.empty? && @order.first.first == :text
        stream.puts "#{@banner}\n" if @banner
      else

        if @name_desc
          stream.puts 'NAME'.colorize(:red)
          stream.puts leftcol_spaces.to_s + colorize_known_words(eval(%("#{@name_desc}"))) + "\n"
          stream.puts
        end
        ...
        banner = colorize_known_words_array(wrap(eval(%("#{@banner}")) + "\n", prefix: leftcol_start)) if @banner # lazy banner
        stream.puts banner
      ...
      @order.each do |what, opt|
        ...
        stream.puts wrap(format('      %s', desc), prefix: leftcol_start, width: width - rightcol_start - 1)
        stream.puts
        stream.puts
      end
      end
    ...
    def educate_short(stream = $stdout)
      ...
      if !@order.empty? && @order.first.first == :text
        ...
        stream.puts "#{@version}\n" if @version
        stream.puts colorize_known_words_array(wrap(eval(%("#{@banner}")) + "\n", prefix: leftcol_start)) if @banner # jit banner
        stream.puts 'Options:'
      end

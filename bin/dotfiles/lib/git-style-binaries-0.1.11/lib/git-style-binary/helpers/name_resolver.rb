...
module GitStyleBinary
  module Helpers
    module NameResolver
      ...
      def current_command_name(_filename = zero, _argv = ARGV)
      ...
      def full_current_command_name(filename = zero, argv = ARGV)
        ...
        subcmd = cur != basename(filename) # is this a subcmd?
        format('%s%s%s', basename(filename), subcmd ? '-' : '', subcmd ? current_command_name(filename, argv) : '')
      end
      ...
      def pretty_known_subcommands(theme = :long)
        GitStyleBinary.known_commands.collect do |k, cmd|
          ...
          cmd.process_parser!
          format('%-s%s%-10s', basename, '-', k).colorize(:light_blue) + format('%s       ', theme == :long ? "\n" : '') + format('%s', cmd.short_desc) + "\n"
        end.compact.sort
      end
      end
  end

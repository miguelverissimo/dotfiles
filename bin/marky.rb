...
optparse = OptionParser.new do |opts|
  ...
  opts.on('-o DIR', '--output DIR', 'Output folder, default STDOUT. Use "." for current folder, void if output type is "nv"') do |outfolder|
    ...
    $options[:outfolder] = filepath.gsub(%r{/$}, '') + '/'
...
def html_file_to_markdown(outtype)
  $input.each do |file|
    ...
    if File.exist?(input)
      html = File.open(input, 'r') do |infile|
...
def url_to_markdown
  $input.each do |input|
    ...
    if res.code.to_i == 200
      if $options[:outfolder]
        outfile = $options[:outfolder]
        outfile += input.gsub(%r{^https?://}, '').strip.gsub(%r{/}, '_').gsub(/[!?*$^()]+/, '') + '.md'
        File.open(outfile, 'w') do |f|
          f.puts res.body
        end
        puts "Markdown written to #{outfile}"
...
def html_to_nv(input)
  ...
  if res.code.to_i == 200
    `osascript -e 'tell app "nvALT" to activate'`
    `open "#{res.body}"`
  else
    puts 'Error converting HTML'
  end
...
def webarchive_url_to_markdown(outtype)
  $input.each do |f|
    ...
    source_url = `mdls -name 'kMDItemWhereFroms' -raw #{file}`.split("\n")[1].strip.gsub(/(^"|"$)/, '')
    ...
    if res.code.to_i == 200
      if outtype == 'nv'
        ...
        outfile = $options[:outfolder]
        outfile += `textutil -info #{file} | grep "Title:"`.gsub(/^\s*Title:\s*/, '').strip.gsub(/[!?*$^()]+/, '') + '.md'
        File.open(outfile, 'w') do |f|
          f.puts res.body
        end
...
def webarchive_to_markdown(outtype)
  $input.each do |f|
    file = File.expand_path(f)
    html = `textutil -convert html -noload -nostore -stdout #{file} 2> /dev/null`
...
def bookmark_to_markdown(outtype)
  $input.each do |f|
    ...
    outfile = $options[:outfolder] || ''
    outfile += `mdls -name 'kMDItemDisplayName' -raw "#{file}"`.strip.gsub(/(\.webbookmark|\.webloc)$/, '') + '.md'
    source_url = `mdls -name 'kMDItemURL' -raw "#{file}"`.strip
    if source_url.nil? || source_url == '(null)'
      source_url = File.open(file, 'r') do |infile|
        ic = Iconv.new('UTF-8//IGNORE', 'UTF-8')
        urlstring = ic.iconv(infile.read + ' ')[0..-2].match(%r{\<key\>URL\</key\>\n\s*\<string\>(.*?)\</string\>})
